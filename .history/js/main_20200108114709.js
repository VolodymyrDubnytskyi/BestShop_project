document.addEventListener('DOMContentLoaded', function () {
    ...
    const eventForQuantity = function (e) {
        ...
        if (quantity.value === '' || quantity.value === null || quantity.value === undefined || quantity.value <= 0) {
            ...
            let lastElOfArray = iteamFromStorage.quantity[quantity.length -1];
            console.log(lastElOfArray[lastElOfArray.length -1]);
            let sum = (parseFloat(newElSpan.innerText) + sumQuantity) + iteamFromStorage.quantity[quantity.length -1];
            console.log(sum);
