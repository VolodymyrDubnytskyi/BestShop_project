document.addEventListener('DOMContentLoaded', function () {
    ...
    const eventForQuantity = function (e) {
        ...
        if (quantity.value === '' || quantity.value === null || quantity.value === undefined || quantity.value <= 0) {
            ...
            // let lastElOfArray = iteamFromStorage.quantity[iteamFromStorage.quantity.length -1];
            // console.log(lastElOfArray)
            let sum = parseFloat(newElSpan.innerText) + sumQuantity;
            let totalInnerTex = sum - iteamFromStorage.quantity[iteamFromStorage.quantity.length -2];
            console.log(totalInnerTex);
            console.log(iteamFromStorage.quantity);
            newElSpan.innerText = totalInnerTex;
