document.addEventListener('DOMContentLoaded', function () {
    ...
    const eventForOrders = function (e) {
        ...
        if (inMonth.value === '' || inMonth.value === null || inMonth.value === undefined || inMonth.value <= 0) {
            ...
            // const suminMonth = inMonth.value * perDady;
            ...
            for (const el of spanClick) {
                el.addEventListener('click', function (e) {
                    ...
                    if (select.value === "professional") {
                        count = 25;
                        const ordersSum  = document.querySelector(".ordersSum").innerText;
                        const selectPackageSum  = document.querySelector(".selectPackageSum").innerText;
                        const accountingSum  = document.querySelector(".accountingSum").innerText;
                        const terminalSum  = document.querySelector(".terminalSum").innerText;
                        const productsSum  = document.querySelector("#productsSum").innerText;
                        console.log(parseFloat(terminalSum));
                        let res = parseFloat(productsSum) + parseFloat(ordersSum) + parseFloat(selectPackageSum) + parseFloat(accountingSum) + parseFloat(terminalSum) + parseFloat(count);
                        // let sumOfTotal = newElSpan.innerText;
                        // let newSumOfTotal = parseFloat(sumOfTotal);
                        console.log(res);
                        newElSpan.innerText = res;
